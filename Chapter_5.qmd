---
title: "Chapter 5: Core Data Structures"
format: html
editor: visual
---
...
Data structures are ways of organizing and storing data so that it can be accessed and used efficiently. Python provides several built-in data structures that make it easy to store and manipulate collections of data.

The most common core data structures in Python are:
...
Each structure has different properties and is used for different purposes.

::: callout-tip
## Learning Objectives

After studying this lession, you should be able to:

-   **Identify and describe Python’s core data structures**: lists, tuples, dictionaries, and sets.
-   **Create and manipulate these data structures** to store and organize data effectively.
-   **Explain the differences** between the four core data structures.
-   **Work with nested data structures**, such as lists inside lists or dictionaries inside dictionaries.
-   **Access and modify elements** within different data structures using indexing or keys.
-   **Choose the appropriate data structure** based on the type of data and the problem being solved.
:::
...
```{python}
{1,2}.add([3,4])  # ❌ lists cannot be elements
```
:::
...
**Definition**: A dictionary in Python is an unordered, mutable collection of key–value pairs. Each key must be unique and immutable, while values can be of any data type.
...
-   dict\[key\] → error if key missing
-   dict.get(key) → returns None (or default)
...
# If it exists → returns its current value.
# If it does NOT exist → returns 0 (default value).
# Then we add 1 to it.
...
**Definition**: The bytes data type in Python represents a sequence of raw binary data. It is an immutable sequence of integers where each value must be in the range 0–255.
...
| Byte values | Each element is integer (0–255)   |
...
-   **Letters**: 'A' to 'Z' (uppercase) are represented by ASCII values 65–90, and 'a' to 'z' (lowercase) by values 97–122.
-   **Digits**: '0' to '9' are represented by ASCII values 48–57.
...
-   **Variable-Length Encoding**:
    -   In UTF-8, the number of bytes used for a character depends on the character's code point in Unicode:
        -   1 byte (7 bits) for characters in the ASCII range (0–127).
...
-   **Compatibility with ASCII**: Since UTF-8 is compatible with ASCII for characters in the 0–127 range, it's easy to integrate into systems that were originally built to handle only ASCII.
-   **Compact**: It’s relatively compact for languages that mainly use ASCII characters (like English), while still being able to handle any character in the world.
...
-   The character **'A'** has a Unicode code point of **U+0041**. In UTF-8, it is represented by a single byte:
    -   `41` (hexadecimal) = `'A'`
-   The character **'€'** (Euro symbol) has a Unicode code point of **U+20AC**. In UTF-8, it is represented by three bytes:
    -   `E2 82 AC` (hexadecimal) = `'€'`
-   The character **'𐍈'** (an ancient Gothic letter) has a Unicode code point of **U+10348**. In UTF-8, it is represented by four bytes:
    -   `F0 90 8D 88` (hexadecimal) = `'𐍈'`

**Summary of UTF-8:**

-   **Variable-length encoding**: UTF-8 can use 1 to 4 bytes per character.
-   **Compatibility with ASCII**: For characters in the ASCII range (0–127), UTF-8 is identical to ASCII.
...
| Feature | ASCII | UTF-8 |
|-------------------|-------------------------|----------------------------|
| **Character Range** | Limited to 128 characters (0–127) | Supports over 1.1 million characters |
...
-   **Basic Compatibility**: UTF-8 is fully compatible with ASCII for characters in the range 0–127 (the standard ASCII range).
-   **Standardized Encoding**: Both are standardized character encodings designed to facilitate the representation of text.
-   **Character Representation**: Both ASCII and UTF-8 represent characters as numeric codes (ASCII for 0–127, UTF-8 for all Unicode characters).
